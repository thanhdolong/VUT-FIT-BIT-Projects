...
**/
	...
	if (*(RootPtr) == NULL)
	{
		tBSTNodePtr NewPtr = malloc (sizeof( struct tBSTNode ));
	...
	{ // Je root shodny s klicem
		(* RootPtr) -> BSTNodeCont = Content;
	} else if ((* RootPtr) -> Key < K)
	{ // hledani v pravem podstromu
		BSTInsert (&(* RootPtr)->RPtr, K, Content);
	} else if ((* RootPtr) -> Key > K)
	{  // hledani v levem podstromu
...
**/
	...
	if ((* RootPtr) -> Key == K)
	{ // Uzel, který má být smazán
		tBSTNodePtr pomocny = *RootPtr;
		if ((* RootPtr) -> RPtr == NULL ) 
		{ // Ruseni uzel nema pravy podstrom
			*RootPtr = pomocny->LPtr;
			free(pomocny);
		} else if ((* RootPtr) -> LPtr == NULL )
		{ // Ruseni uzel nema pravy podstrom
			*RootPtr = pomocny->RPtr;
			free(pomocny);		
		} else
		{ // Ruseni uzel ma oba podstromy
