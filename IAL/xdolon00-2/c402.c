...
**/
    if (*RootPtr == NULL)
    {
        ...
        {
            //Error();
            printf("Nepodarilo se naalokovat misto");//stderror
            return;
        }
        ...
        tBTNodePtr TargetNode = NULL;

        while( ActNode != NULL ) {  // cyklus na nájdenie najbližšej hodnoty
            TargetNode = ActNode;
            if ( ActNode->Cont > Content )  // vstup do ľavého syna
                ActNode = ActNode->LPtr;
            else if ( ActNode->Cont < Content ) // vstup do pravého syna
        ...
        if ( TargetNode->Cont < Content )   // Novy list bude vlozeny vpravo
            TargetNode->RPtr = NewNode;
        else
            TargetNode->LPtr = NewNode;
...
**/
    ...
    while (SEmptyP(&stack) == false) {
        RootPtr = STopPopP(&stack); // nacteni prvku ze zasobniku
        BTWorkOut(RootPtr); // volani pri zpracovani uzlu stromu
        if (RootPtr->RPtr != NULL) {
            // pokud vpravo neni NULL, dale doprava
...
**/
    ...
    SInitB(&stackB); // inicializace bool zasobniku

    Leftmost_Postorder(RootPtr, &stackP, &stackB);
    while (SEmptyP(&stackP) == false) {
        RootPtr = STopPopP(&stackP); // nacteni uzlu
        SPushP(&stackP, RootPtr);// vraceni uzlu do zasobniku
        if (STopPopB(&stackB)) { // overeni, jestli jit zprava nebo zleva
            SPushB(&stackB, false); // vlozeni priznaku "priste prijde zprava"
            Leftmost_Postorder(RootPtr->RPtr, &stackP, &stackB);
        } else { // v uzlu uz byl
            STopPopP(&stackP); // popnuti zasobniku
            BTWorkOut(RootPtr); // volani pri zpracovani uzlu
        }
    }
}
