...
void phong_fragmentShader(
  ...
  (void)gpu;
    ...
    Vec3 diffuse_color;
    init_Vec3(&diffuse_color, 0.f, 1.f, 0.f);

    Vec3 specular_color;
    init_Vec3(&specular_color, 1.f, 1.f, 1.f);

    float specularIntensity;

    if (lambertian > 0.f) specularIntensity = powf(dot_Vec3(&R, &V), 40.f);
    else specularIntensity = 0.f;

    Vec3 diffuse;
    multiply_Vec3_Float(&diffuse, &diffuse_color, lambertian);

    Vec3 specular;
    multiply_Vec3_Float(&specular, &specular_color, specularIntensity);
